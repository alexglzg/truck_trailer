...
class DCBFNode(Node):
    def __init__(self):
        ...
        # Goal Subscriber (Robust QoS for RViz interaction)
        qos_goal = QoSProfile(
        ...
        # Default Target (will be overwritten by RViz)
        ...
        self.setup_mpc()
        self.get_logger().info("MPC Ready. Waiting for State & Goal...")
    ...
    def setup_mpc(self):
        ...
        for k in range(self.N):
            ...
            if k < self.N_cbf:
                Lambda_tractor.append(opti.variable(4)); Mu_tractor.append(opti.variable(4)); Omega_tractor.append(opti.variable(1))
                Lambda_trailer.append(opti.variable(4)); Mu_trailer.append(opti.variable(4)); Omega_trailer.append(opti.variable(1))
        X.append(opti.variable(4))
        ...
        for k in range(self.N):
            ...
            if k < self.N_cbf:
                ...
                # Tractor CBF
                opti.subject_to(Lambda_tractor[k] >= 0); opti.subject_to(Mu_tractor[k] >= 0); opti.subject_to(Omega_tractor[k] >= 0)
                ...
                opti.subject_to(ca.mtimes(A_tractor_dm.T, Mu_tractor[k]) + ca.mtimes([R_trac.T, ca.mtimes([self.par_A_obs.T, Lambda_tractor[k]])]) == 0)
                ...
                # Trailer CBF
                opti.subject_to(Lambda_trailer[k] >= 0); opti.subject_to(Mu_trailer[k] >= 0); opti.subject_to(Omega_trailer[k] >= 0)
                ...
                opti.subject_to(ca.mtimes(A_trailer_dm.T, Mu_trailer[k]) + ca.mtimes([R_trail.T, ca.mtimes([self.par_A_obs.T, Lambda_trailer[k]])]) == 0)
    ...
    def state_cb(self, msg):
        d = msg.data
        self.current_state = np.array([d[0], d[1], d[3], d[2]]) # [x1, y1, th1, th0]
    ...
    def publish_path(self, sol):
        ...
        for k in range(self.N + 1):
            ...
            pose.pose.orientation.w = 1.0 # Simple visual, yaw not needed for line
