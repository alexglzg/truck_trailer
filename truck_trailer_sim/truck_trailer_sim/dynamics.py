...
class Dynamics:
    ...
    def step(self, states: torch.Tensor, actions: torch.Tensor, t: int) -> torch.Tensor:
        ...
        u = [vk, wk]
        """
        x1, y1, th0, th1, phi = states[:, 0], states[:, 1], states[:, 2], states[:, 3], states[:, 4]
        vk, wk = actions[:, 0], actions[:, 1]
        ...
        V0 = vk * torch.cos(phi)  # truck velocity
        w0 = vk * torch.sin(phi) / self._L0  # truck angular velocity

        # Trailer velocity (kinematic relation)
        # V1 = V0 * torch.cos(beta) + self._M0 * (V0 * torch.tan(delta0) / self._L0) * torch.sin(beta)
        V1 = V0 * torch.cos(beta) + self._M0 * w0 * torch.sin(beta)
        ...
        dth0 = w0
        dth1 = (V0 * torch.sin(beta)) / self._L1 - \
               (self._M0 * w0 * torch.cos(beta)) / self._L1
        dphi = wk        
